    ...
    pool.query(
      ...
      (err, result) => {
        ...
        bcrypt.hash(password, saltOrRounds, (err, hash) => {
          ...
          pool.query(
            "INSERT INTO client (iduser, nom, prenom,email) VALUES (?, ?, ?, ?)",
...
app.post("/api/v1/admin/register", async (req, res) => {
  try {
    ...
    await pool.query(
      "INSERT INTO admine (adminid, email, password) VALUES (?, ?, ?)",
...
app.post("/api/v1/admin/login", async (req, res) => {
  try {
    ...
    const { adminid, password: hashedPassword } = result[0];
    ...
    const token = generateJwt({ adminId: adminid, admin });
    res.setHeader("Authorization", `Bearer ${token}`);
    res.json({ token: `Bearer ${token}`, bienvenue: `Nom: ${admin}` });
