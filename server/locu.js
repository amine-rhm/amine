...
app.get("/api/v1/info/pro/annonce/:id", async (request, response) => {
  try {
    ...
    await pool.query(
      ` SELECT CONCAT(',',
      CASE
      WHEN bien.type = 'Terrain' THEN CONCAT(terrain.categorie, ',', terrain.largeur, ',', terrain.longueur, ',', terrain.ville, ',', terrain.adresse)
      WHEN bien.type = 'Industriel' THEN CONCAT(industriel.puissance, ',', industriel.materiel, ',', industriel.taille)
      WHEN bien.type = 'Résidentiel' AND résidentiel.type_residence = 'Maison' THEN CONCAT(maison.etage_maison, ',', résidentiel.meuble, ',', résidentiel.ville, ',', résidentiel.équipement, ',', résidentiel.adresse, ',', résidentiel.type_residence)
      WHEN bien.type = 'Résidentiel' AND résidentiel.type_residence = 'Villa' THEN CONCAT(villa.etage_villa, ',', villa.type_villa, ',', résidentiel.meuble, ',', résidentiel.ville, ',', résidentiel.équipement, ',', résidentiel.adresse, ',', résidentiel.type_residence)
      WHEN bien.type = 'Résidentiel' AND résidentiel.type_residence = 'Studio' THEN CONCAT(studio.idStu, ',', résidentiel.meuble, ',', résidentiel.ville, ',', résidentiel.équipement, ',', résidentiel.adresse, ',', résidentiel.type_residence)    
      WHEN bien.type = 'Résidentiel' AND résidentiel.type_residence = 'Appartement' THEN CONCAT(appartement.type_appartement, ',', résidentiel.meuble, ',', résidentiel.ville, ',', résidentiel.équipement, ',', résidentiel.adresse, ',', résidentiel.type_residence)    
      WHEN bien.type = 'Commercial' THEN CONCAT(commercial.equipement, ',', commercial.etage, ',', commercial.ville, ',', commercial.adresse)
        ELSE ''
      END) AS selected_data,
      annonce.titre, 
      annonce.description, 
      ...
      bien.idann,
      résidentiel.meuble AS résidentiel_meuble,
      résidentiel.ville AS résidentiel_ville,
      résidentiel.équipement AS résidentiel_équipement,
      résidentiel.adresse AS résidentiel_adresse,
      résidentiel.type_residence AS résidentiel_residence,
      commercial.equipement AS commercial_equipement,
      commercial.etage AS commercial_etage,
      commercial.ville AS commercial_ville,
      commercial.meuble AS commercial_meuble,
      commercial.adresse AS commercial_adresse,
      industriel.capacite AS industriel_capacite,
      industriel.puissance AS industriel_puissance,
      industriel.materiel AS industriel_materiel,
      industriel.taille  AS industriel_taille,
      industriel.ville  AS industriel_ville,
      industriel.meuble AS industriel_meuble,
      industriel.adresse AS industriel_adresse,
      terrain.categorie AS terrain_categorie,
      terrain.largeur AS terrain_largeur,
      terrain.longueur AS terrain_longueur,
      terrain.ville AS terrain_ville,
      terrain.meuble AS terrain_meuble,
      terrain.adresse AS terrain_adresse,
      ...
      (err, result) => {
        ...
        if (result[0].type === 'Terrain') {
          formattedData.bien_details.terrain = removeNullValues({
            ...
            largeur: result[0].terrain_largeur,
            longueur: result[0].terrain_longueur,
            ville: result[0].terrain_ville,
            meuble: result[0].terrain_meuble,
            adresse: result[0].terrain_adresse
          });
         } 
  else if (result[0].type === 'Industriel') {
          formattedData.bien_details.industriel = removeNullValues({
            ...
            materiel: result[0].industriel_materiel,
            taille: result[0].industriel_taille,
            ville: result[0].industriel_ville,
            meuble: result[0].industriel_meuble,
            adresse: result[0].industriel_adresse
          });
          } 
 else if (result[0].type === 'Résidentiel'&& result[0].résidentiel_residence === 'Villa') {
          formattedData.bien_details.résidentiel = removeNullValues({
            meuble: result[0].résidentiel_meuble,
            ville: result[0].résidentiel_ville,
            équipement: result[0].résidentiel_équipement,
            adresse: result[0].résidentiel_adresse,
            type_residence: result[0].résidentiel_residence,
          ...
          formattedData.bien_details.commercial = removeNullValues({
            equipement: result[0].commercial_equipement,
            etage: result[0].commercial_etage,
            ville: result[0].commercial_ville,
            meuble: result[0].commercial_meuble,
            adresse: result[0].commercial_adresse
          });
          }
        response.json(formattedData);
      }
